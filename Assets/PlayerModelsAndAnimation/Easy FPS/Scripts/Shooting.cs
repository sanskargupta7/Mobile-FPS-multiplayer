    ...
    public void Fire()
    {
        ...
        if(Physics.Raycast(ray, out _hit, 100))
        {
            ...
            if(_hit.collider.gameObject.CompareTag("Player") && !_hit.collider.gameObject.GetComponent<PhotonView>().IsMine)
            {
                ...
                _hit.collider.gameObject.GetComponent<PhotonView>().RPC("TakeDamage", RpcTarget.AllBuffered, 10f);           //to access RPC() we need to first access clients photonview component.....as RPC allows us to broadcast methods for all remote clients in the same room
